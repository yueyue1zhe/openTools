      ...
      <el-form :label-width="props.labelWidth" label-position="left">
        <template v-for="(item, key) in useOpts" :key="key">
          <el-form-item :label="showLabel(item)">
            <div
              v-if="item.type === YlEasyFormItemTypeState.powerSort.value"
            ...
            <template v-if="item.type === YlEasyFormItemTypeState.text.value">
              <easy-form-input
                :opt="item"
                v-model="out"
            ...
            <template v-if="item.type === YlEasyFormItemTypeState.number.value">
              <el-input-number v-model="out[item.name]"></el-input-number>
            </template>
            <template
              v-if="item.type === YlEasyFormItemTypeState.uploadImage.value"
            >
              <y-upload-image v-model="out[item.name]"></y-upload-image>
            </template>
            <template v-if="item.type === YlEasyFormItemTypeState.switch.value">
              <el-switch v-model="out[item.name]"></el-switch>
              <span v-if="item.placeholder" class="y-desc y-m-l-5">{{
                item.placeholder
              }}</span>
            </template>
            <template v-if="item.type === YlEasyFormItemTypeState.radio.value">
...
import { YlEasyFormItemTypeState } from "@/components/yueyue-ui/components/yl-form/State";
import YUploadImage from "@/components/yueyue-ui/components/yl-file-upload/YlUploadImage.vue";
import { ElMessage } from "element-plus";
import EasyFormInput from "@/components/yueyue-ui/components/yl-form/easyFormInput.vue";
import { easyFormGetProperty } from "@/components/yueyue-ui/components/yl-form/easyFormTools";
...
interface propsTypes {
  ...
  opts: YlEasyFormTypes.OptsItemType[];
}

const useOpts = computed(() => {
  let useOpts: YlEasyFormTypes.OptsItemType[] = [];
...
const showLabel = (item: YlEasyFormTypes.OptsItemType): string => {
  return item.label ? item.label : item.name;
};
const showPlaceholder = (item: YlEasyFormTypes.OptsItemType) => {
  if (!item.placeholder) {
    let outStr = "请输入";
    switch (item.type) {
      case YlEasyFormItemTypeState.text.value:
        outStr += showLabel(item);
        break;
      case YlEasyFormItemTypeState.uploadImage.value:
        outStr = "请上传" + showLabel(item);
        break;
      case YlEasyFormItemTypeState.switch.value:
