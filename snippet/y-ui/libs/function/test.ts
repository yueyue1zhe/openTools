 ...
 * 验证日期格式
 */
function date(value: string | Date | number) {
 ...
 * 只能是字母或者数字
 */
function enOrNum(value: string | number) {
    if (typeof value == "number") {
        value = String(value)
    }
    //英文或者数字
 ...
 * 验证是否包含某个值
 */
function contains(value: string | [any], param: never) {
 ...
 * 验证一个值范围[min, max]
 */
function range(value: string | number, param: [number, number]) {
